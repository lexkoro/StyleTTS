...
class Trainer(object):
    ...
    def get_image(arrs):
        ...
        width = 0
        for arr in arrs:
            uint_arr = (((arr - arr.min()) / (arr.max() - arr.min())) * 255).astype(
                np.uint8
            )
    ...
    def run(self, batch):
        ...
        mel_input_length = torch.div(
            mel_input_length, (2**self.model.n_down), rounding_mode="floor"
        )
    ...
    def _train_epoch(self):
        ...
        for train_steps_per_epoch, batch in enumerate(
            (pbar := tqdm(self.train_dataloader)), 1
        ):
            ...
            pbar.set_description(
                f"[Loss: {losses['loss']:.4f}, LR: {self._get_lr():.5f}]"
            )
