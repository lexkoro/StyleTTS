        ...
        for i, batch in enumerate(train_dataloader):
            ...
            if epoch > TMA_epoch:  # start TMA training
                ...
                if TMA_CEloss:
                    ...
                    log_attn = torch.nan_to_num(
                        F.log_softmax(s2s_attn_feat, dim=1)
                    )  # along the mel dimension
                    loss_mono = -(
                        torch.mul(log_attn, s2s_attn_mono).sum(axis=[-1, -2])
                        / input_lengths
                    ).mean()
                else:
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(val_dataloader):
                ...
                with torch.no_grad():
                    ...
                    m = length_to_mask(input_lengths)
                    ppgs, s2s_pred, s2s_attn_feat = model.text_aligner(
                        mels, mask, texts
                    )
                ...
                with torch.no_grad():
                    ...
                    F0 = F0.reshape(
                        F0.shape[0], F0.shape[1] * 2, F0.shape[2], 1
                    ).squeeze()
