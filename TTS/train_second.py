    ...
    for epoch in range(epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            with torch.no_grad():
                ...
                if TMA_CEloss:
                    s2s_attn = F.softmax(
                        s2s_attn_feat, dim=1
                    )  # along the mel dimension
                else:
                    s2s_attn = F.softmax(
                        s2s_attn_feat, dim=-1
                    )  # along the text dimension
            ...
            _, p = model.predictor(t_en, s, input_lengths, s2s_attn_mono, m)
            ...
            for _s2s_pred, _text_input, _text_length in zip(d, d_gt, input_lengths):
                loss_dur += F.l1_loss(
                    _s2s_pred[1 : _text_length - 1], _text_input[1 : _text_length - 1]
                )
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(val_dataloader):
                ...
                d, p = model.predictor(t_en, s, input_lengths, s2s_attn_mono, m)
                # get clips
                mel_len = int(mel_input_length.min().item() / 2 - 1)
                en = []
                gt = []
